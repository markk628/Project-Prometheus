...
from src.config.config import DATA_DIR, CUTOFF_TIMESTAMP, REGIME_TICKERS, MASSIVE_APIKEY, V7_BASKET
...
class DataFeatureEngineer:
    def __init__(
        ...
        self.tickers_dir      = Path(self.data_dir) / "preprocessed" / "v7" / "tickers"
    ...
    def _build_unified(self, ticker_dfs: Dict[str, pl.DataFrame]) -> None:
        ...
        Output
        ------
        <data_dir>/preprocessed/v7/unified/unified.parquet
        """

        # ------------------------------------------------------------------
        # TODO(v7): add long-horizon regime levels + medium-scale deltas
        #
        ...
        # ------------------------------------------------------------------
        # ------------------------------------------------------------------
        # Window choice (post-v7 redesign per the long-horizon plan).
        ...
        if spy_col not in stacked.columns:
            ...
            # v7: basket members are regime tickers that are ALSO trained on,
            # so they need _rs_spy_* features. tradable_tickers excludes them
            # (correctly, for breadth/CS); compute RS over the union here.
            # SPY_rs_spy_*d is trivially zero (SPY vs SPY) — harmless, ignored.
            basket_in_present = [t for t in V7_BASKET if t in tickers_present]
            rs_target_tickers = tradable_tickers + basket_in_present

            rs_exprs = []
            for w in RS_WINDOWS:
                spy_ret = pl.col(spy_col) if w == 1 else pl.col(spy_col).rolling_sum(window_size=w)

                for ticker in rs_target_tickers:
        ...
        # v7: basket members are regime tickers that ARE trained on, so
        # exempt them from the prune. Non-basket regime tickers still get
        # their per-ticker columns dropped as before. SPY_close exception
        # below stays in place — no-op when SPY is in the basket (current
        # asset-class lean), still matters for sector-slice basket where
        # SPY would be regime-only.
        preserve_set = set(V7_BASKET)
        to_prune_tickers = [t for t in regime_tickers_present if t not in preserve_set]

        if to_prune_tickers:
            n_preserved = len(preserve_set & set(regime_tickers_present))
            if self.logger:
                self.logger.info(
                    f"  Pruning regime-ticker columns "
                    f"({len(to_prune_tickers)} of {len(regime_tickers_present)} "
                    f"regime tickers; {n_preserved} basket members preserved)..."
                )

            n_before = len(stacked.columns)
            regime_prefixes = tuple(f"{t}_" for t in to_prune_tickers)
            cols_to_drop = [
                c for c in stacked.columns
        ...
        out_dir = Path(self.data_dir) / "preprocessed" / "v7" / "unified"
