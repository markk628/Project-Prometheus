...
class Environment:
    def __init__(
        ...
        market_close_idx = np.roll(market_open_idx, -1) - 1
        market_close_idx[-1] = len(data) - 1
    ...
    def _calculate_reward(self, prev_portfolio_value: float, current_portfolio_value: float) -> float:
        ...
        # trade_penalty = 0.0003 * (abs(self.shares_traded) / self.max_trading_units)
...
def main():
    ...
    ticker = 'TSLA'
    data_dir = f'{DATA_DIR}/preprocessed/v3/{ticker}/{ticker}_train.csv'
