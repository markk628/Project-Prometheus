...
class Environment:
    def __init__(
        ...
        # A new trading day starts where the calendar date changes
        date_changes = np.where(ts_dates[1:] != ts_dates[:-1])[0] + 1
        market_open_idx = np.concatenate([[0], date_changes])
        market_close_idx = np.concatenate([date_changes - 1, [len(data) - 1]])
        self.market_open_idx = market_open_idx[:-MULTIDAY_MINUTE_EPISODE_DAYS]
        # self.market_close_idx = market_close_idx[:-MULTIDAY_MINUTE_EPISODE_DAYS]
        self.market_close_idx = market_close_idx
    ...
    def _get_episode_bounds(self, days: int=MULTIDAY_MINUTE_EPISODE_DAYS) -> Tuple[int, int, int]:
        """
        Returns (episode_start, episode_end, episode_length)
        """
        # Find which trading day idx
        day_idx = np.searchsorted(self.market_open_idx, self.current_step, side="right") - 1
        
        episode_start = self.market_open_idx[day_idx]
        episode_end = self.market_close_idx[day_idx + days - 1]
        episode_length = episode_end - episode_start + 1
        
        return episode_start, episode_end, episode_length
       
    def reset(self, days: int=MULTIDAY_MINUTE_EPISODE_DAYS) -> Dict[str, np.ndarray]:
        # State
        self.current_step_in_episode = 0
        ...
        self.invalid_actions_count = 0
        self.episode_start, self.episode_end, self.episode_length = self._get_episode_bounds(days)
        # self.current_action = None
        # self.previous_action = None
        # self.current_action = 0
        # self.previous_action = 0
    ...
    def _get_observation(self) -> Dict[str, np.ndarray]:
        ...
        portfolio_state = np.array([
            ...
            min(self.hold_time / self.episode_length, 1.0),          # hold time ratio [0, 1]
    ...
    def _get_portfolio_value(self) -> float:
        ...
        conservative_price = self._get_current_price() - (self.spread / 2) - (self._get_current_price() * self.slippage)
        return self.balance + (self.shares_held * conservative_price)
    ...
    def _calculate_reward(self, prev_portfolio_value: float, current_portfolio_value: float) -> float:
        ...
        # portfolio_return = (current_portfolio_value - prev_portfolio_value) / prev_portfolio_value
        # transaction_penalty = (self.current_transaction_fee * abs(self.shares_traded_at_step)) / current_portfolio_value
        ...
        # # Ensure shares_traded_at_step is relative (e.g., 0.1 means 10% of portfolio moved)
        # transaction_penalty = (self.current_transaction_fee * abs(self.shares_traded_at_step)) / current_portfolio_value
        ...
        # trade_penalty = 0.0003 * (abs(self.shares_traded_at_step) / self.max_trading_units)

        # # if self.previous_action is not None:
        # #     action_change_penalty = 0.0002 * abs(self.current_action - self.previous_action)
        ...
        # reward = portfolio_return - trade_penalty - action_change_penalty
...
def main():
    ...
    ticker = 'TSLA'
    data_dir = f'{DATA_DIR}/preprocessed/v4/{ticker}/{ticker}_train.csv'
